...
func main() {
	...
	case "decodeVersionedNonce":
		...
		utils.CheckErr(err, "Error encoding output")
	...
	case "encodeCrossDomainMessage":
		...
		encoded, err := utils.EncodeCrossDomainMessage(nonce, sender, target, value, gasLimit, data)
		utils.CheckErr(err, "Error encoding cross domain message")
	...
	case "hashDepositTransaction":
		...
		depositTx := utils.MakeDepositTx(from, to, value, mint, gasLimit, false, data, l1BlockHash, logIndex)

		// RLP encode deposit transaction
		encoded, err := types.NewTx(&depositTx).MarshalBinary()
		utils.CheckErr(err, "Error encoding deposit transaction")
	...
	case "encodeDepositTransaction":
		...
		depositTx := utils.MakeDepositTx(from, to, value, mint, gasLimit, isCreate, data, l1BlockHash, logIndex)

		// RLP encode deposit transaction
		encoded, err := types.NewTx(&depositTx).MarshalBinary()
		utils.CheckErr(err, "Failed to RLP encode deposit transaction")
	...
	case "hashWithdrawal":
		...
		// Hash withdrawal
		hash, err := utils.HashWithdrawal(nonce, sender, target, value, gasLimit, data)
		utils.CheckErr(err, "Error hashing withdrawal")
	...
	case "hashOutputRootProof":
		...
		hash, err := utils.HashOutputRootProof(version, stateRoot, messagePasserStorageRoot, latestBlockHash)
		utils.CheckErr(err, "Error hashing output root proof")
	...
	case "getProveWithdrawalTransactionInputs":
		...
		wdHash, err := utils.HashWithdrawal(nonce, sender, target, value, gasLimit, data)
		utils.CheckErr(err, "Error hashing withdrawal")
		...
		utils.CheckErr(err, "Error packing withdrawal slot")
		...
		utils.CheckErr(err, "Error creating secure trie")
		...
		utils.CheckErr(account.EncodeRLP(writer), "Error encoding account")
		world.UpdateStorage(common.Address{}, predeploys.L2ToL1MessagePasserAddr.Bytes(), writer.Bytes())

		// Get the proof
		var proof utils.ProofList
		utils.CheckErr(state.Prove(predeploys.L2ToL1MessagePasserAddr.Bytes(), 0, &proof), "Error getting proof")

		// Get the output root
		outputRoot, err := utils.HashOutputRootProof(common.Hash{}, world.Hash(), state.Hash(), common.Hash{})
		utils.CheckErr(err, "Error hashing output root proof")
